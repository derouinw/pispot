...
class Commander(cmd.Cmd):
    ...
    def __init__(self):
        ...
        self.playlist = "spotify:user:123590429:playlist:0Zlrac00rjVZTRRBoMGBr3" # default to 'good vibes'
        self.track = "spotify:track:2cmRpmO04TLaKPzmAzySYZ" # default to 'dance yrself clean'
        self.index = 0

        self.session.on(spotify.SessionEvent.PLAY_TOKEN_LOST, self.do_resume)
    ...
    def do_relogin(self, line):
        ...
        self.playlist = self.session.get_playlist(self.playlist)
        self.track = self.session.get_track(self.track)
    ...
    def do_playlist(self, query):
        "playlist [uri] (list tracks for the playlist)"
        if query != "":
            self.playlist = self.session.get_playlist(query)

        print self.playlist.load().name
        track = self.playlist.tracks[0]
        print track.load().name,"-",track.artists[0].name
        self.do_play_uri(track.link.uri)

        self.session.on(spotify.SessionEvent.END_OF_TRACK, self.do_next)

    def do_next(self, query):
        self.logger.info('Skipping track')
        self.index += 1
        if self.index >= len(self.playlist.tracks):
            self.index = 0
        track = self.playlist.tracks[self.index]
        print track.load().name,"-",track.artists[0].name
        self.do_play_uri(track.link.uri)

    def do_prev(self, query):
        self.logger.info('Previous track')
        self.index -= 1
        if self.index <= 0:
            self.index = len(self.playlist.tracks)-1
        track = self.playlist.tracks[self.index]
        print track.load().name,"-",track.artists[0].name
        self.do_play_uri(track.link.uri)

    def do_goto(self, query):
        "goto <index>"
        try:
            query = int(query)
        except:
            self.logger.warning('Gimme an index')
            return

        self.index = query
        if self.index >= len(self.playlist.tracks) or self.index < 0:
            self.logger.warning('Index out of bounds')
            return
    ...
    def do_end(self, query):
        self.logger.info('Skipping to the end')
        self.index = len(self.playlist.tracks)-1
        track = self.playlist.tracks[self.index]
        print track.load().name,"-",track.artists[0].name
        self.do_play_uri(track.link.uri)
